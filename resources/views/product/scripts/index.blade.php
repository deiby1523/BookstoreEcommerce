        ...
        $.ajax({
            ...
            .done(function (products, response, xhr) {
                ...
                paginationButtons += `<li class="page-item">
                                    <button class="page-link pag-link ${xhr.getResponseHeader("page") == 1 ? 'disabled' : ''}" tabindex="-1" onclick="get_products('${search == null ? '':search}',${xhr.getResponseHeader("page") - 1});">
                ...
                if (parseInt(xhr.getResponseHeader("numPages")) <= 15) {
                    for (let i = 1; i <= xhr.getResponseHeader("numPages"); i++) {
                        paginationButtons += `<li class="page-item ${xhr.getResponseHeader("page") == i ? 'active' : ''}"><button class="page-link" onclick="get_products('${search == null ? '':search}',${i});">${i}</button></li>`;
                    }
                } else {
                    for (let i = 1; i <= 5; i++) {
                        paginationButtons += `<li class="page-item ${xhr.getResponseHeader("page") == i ? 'active' : ''}"><button class="page-link" onclick="get_products('${search == null ? '':search}',${i});">${i}</button></li>`;
                    ...
                    if (parseInt(xhr.getResponseHeader("page")) > 5 && parseInt(xhr.getResponseHeader("page")) < xhr.getResponseHeader("numPages") - 4) {
                        paginationButtons += `<li class="page-item active"><button class="page-link" onclick="get_products('${search == null ? '':search}',${xhr.getResponseHeader("page")});">${xhr.getResponseHeader("page")}</button></li>`;
                        paginationButtons += "......";
                    }
...
<div class="row" style="justify-content: center">
    ...
    <div class="col-sm-1">
        <button id="btnBuscarPag" class="btn btn-sm btn-outline-secondary"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-search" viewBox="0 0 16 16"><path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z"></path></svg></button>
                ...
                paginationButtons += `<li class="page-item">
                                    <button class="page-link pag-link ${xhr.getResponseHeader("page") == xhr.getResponseHeader("numPages") ? 'disabled' : ''}" tabindex="-1" onclick="get_products('${search == null ? '':search}',${parseInt(xhr.getResponseHeader("page")) + 1});">
