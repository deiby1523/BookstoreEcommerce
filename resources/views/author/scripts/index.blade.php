<script>
    ...
    function get_authors(search, page) {
        ...
        $.ajax({
            url: `author/search/${search}`,
            type: 'GET',
            dataType: 'json',
            data: {'search': search, 'page': page},
        })
            .done(function (authors, response, xhr) {
                ...
                authors.forEach(function (author) {
                    resultsList += `<tr>
                                                    <td class='align-middle text-center'><p class=' mb-0'>${author.id}</p></td>
                                                    <td><p class='mb-0 truncated-text-large' data-bs-toggle='tooltip' data-bs-placement='right' title='${author.author_name}'>${author.author_name}</p></td>
                                                    <td class='align-middle'><p class='mb-0'>${author.created_at}</p>
                                                    <td class='align-middle'><p class='mb-0'>${author.updated_at}</p>

                                                    <td class='align-middle' style='text-align: center;'>
                                                        <a href='author/show/${author.id}' class='text-secondary mx-3 font-weight-normal' data-toggle='tooltip' data-original-title='Show user'>
                                                            Visualizar
                                                        </a>
                                                        <a href='author/edit/${author.id}' class='text-secondary mx-3 font-weight-normal' data-toggle='tooltip' data-original-title='Edit user'>
                                                            Editar
                                                        </a>
                                                        <a href='' class='text-secondary font-weight-normal' data-bs-toggle='modal' data-bs-target='#deleteConfirm${author.id}' data-toggle='tooltip' data-original-title='Delete user'>
                                                            Eliminar
                                                        </a>
                                                    </td>
                                                </tr>`;
                ...
                authors.forEach(function (author) {
                    modals += `<div class='modal fade' id='deleteConfirm${author.id}' tabindex='-1' aria-labelledby='deleteConfirm${author.id}' aria-hidden='true'>
                                                    <div class='modal-dialog' style='margin-top: 10rem;'>
                                                        <div class='modal-content'>
                                                            ...
                                                            <div class='modal-body'>
                                                                Esta seguro que desea eliminar el autor '${author.author_name}'?
                                                                <br><br>
                                                                Esta accion es irreversible.
                                                            </div>
                                                            <div class='modal-footer justify-content-between'>
                                                                <button type='button' class='btn bg-gradient-dark mb-0' data-bs-dismiss='modal'>Cancelar</button>
                                                                <form method='DELETE' action='author/delete/${author.id}'>
                ...
                // Insert the complete list of results in #authorDisplay after all authors have been processed
                $("#authorDisplay").html(resultsList);
                $("#modals").html(modals);
...
<div class="row" style="justify-content: center">
    ...
    <div class="col-sm-1">
        <button id="btnBuscarPag" class="btn btn-sm btn-outline-secondary"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-search" viewBox="0 0 16 16"><path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z"></path></svg></button>
                ...
                // $("#infopag").html(`<p class="text-lg"><b>Libros: </b>${xhr.getResponseHeader("numAuthors")}</p>
    ...
    $(document).on('input', '#searchAuthor', function () {
        const searchValue = $('#searchAuthor').val();
